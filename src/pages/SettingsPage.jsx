...
// ---------------- DARK-MODE-AWARE PAGE VARS ----------------
const getPageVars = (darkMode) => ({
...
function SettingsPage() {
  ...
  const pageVars = getPageVars(darkMode);
      ...
      >
        ...
        >
          <div style={{ width: "100%", maxWidth: "min(1320px, 100%)", margin: 0, display: "flex", flexDirection: "column", gap: 12, paddingBottom: 56 }}>
            <SettingsHeader status={status} cardStyle={headerCardStyle} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 14 }}>
              ...
              />
              <AccountInfoCard name={name} username={username} />
              <SecurityCard
...
function SettingsHeader({ status, cardStyle }) {
  return (
    <div className="section-header-card" style={cardStyle}>
      <div
        style={{
          position: "absolute",
          top: 0,
        ...
        {status.message ? (
          <div
            style={{
              ...
              borderRadius: 10,
              padding: "10px 12px",
              background: status.type === "success" ? "var(--success-soft)" : "var(--warning-soft)",
              color: status.type === "success" ? "var(--success)" : "var(--danger)",
              border: status.type === "success" ? "1px solid var(--success-border)" : "1px solid var(--warning-border)",
              fontWeight: 600,
            }}
          >
            {status.message}
...
function ProfilePictureCard({ profilePreview, name, username, savingProfile, onFileChange, onSubmit }) {
  return (
    <div style={CARD_STYLE}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <ProfileAvatar
          ...
          alt="profile"
          style={{ width: 110, height: 110, borderRadius: "50%", objectFit: "cover", border: "3px solid var(--accent-strong)", boxShadow: "var(--shadow-glow)" }}
        />
        <div style={{ flex: 1 }}>
          <input type="file" accept="image/*" onChange={onFileChange} style={{ color: "var(--text-primary)", width: "100%" }} />
          <button onClick={onSubmit} disabled={savingProfile} style={{ ...PRIMARY_BUTTON_STYLE, marginTop: 12, opacity: savingProfile ? 0.7 : 1 }}>
...
function AccountInfoCard({ name, username }) {
  return (
    <div style={CARD_STYLE}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <input type="text" placeholder="Name" value={name} readOnly disabled style={READ_ONLY_INPUT_STYLE} />
        <input type="text" placeholder="Username" value={username} readOnly disabled style={READ_ONLY_INPUT_STYLE} />
        <div
          style={{
            borderRadius: 12,
            padding: "12px 14px",
    ...
    <div style={CARD_STYLE}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <input
          ...
          style={INPUT_STYLE}
        />
        <button onClick={onSubmit} disabled={savingPassword} style={{ ...PRIMARY_BUTTON_STYLE, opacity: savingPassword ? 0.7 : 1 }}>
...
function AppearanceCard({ darkMode, toggleDarkMode, onLogout }) {
  return (
    <div style={CARD_STYLE}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--text-primary)", fontWeight: 800, marginBottom: 14 }}>
      ...
      <button
        ...
        style={{
          ...
          background: darkMode ? "rgba(127,29,29,0.18)" : "#fff",
          color: "var(--danger)",
        }}
      >
        Sign Out
